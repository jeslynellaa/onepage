<x-layout>
    ...
    <div class="mx-auto w-full max-w-7xl px-4 sm:px-6 py-8 space-y-8 antialiased text-gray-900">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-400">
            <div>
                ...
                <h1 class="text-2xl font-black text-gray-900 tracking-tight">Create System Procedure</h1>
            </div>
        </div>

        <div class="space-y-8">
            <!-- MASTER CONTROLLER FORM -->
            <form id="documentForm" method="POST" action="{{ route('document.system_procedures.store') }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                    ...
                    <div class="p-6 space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-12 gap-5">
                            <div class="md:col-span-6">
                                ...
                                <input type="text" id="title" name="title" value="{{ old('title') }}" class="w-full rounded-lg border-gray-200 text-sm p-3 placeholder-gray-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all duration-150" />
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" id="code" name="code" value="{{ old('code') }}" placeholder="e.g., SP-01" class="w-full rounded-lg border-gray-200 text-sm p-3 placeholder-gray-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all duration-150" />
                            </div>

                            <div class="md:col-span-2">
                                <label for="revision_number" class="block text-xs font-bold uppercase text-gray-500 tracking-wide mb-1.5">Revision No.</label>
                                <input readonly type="text" id="revision_number" name="revision_number" placeholder="0" class="w-full rounded-lg border-gray-200 bg-gray-50 text-gray-400 font-semibold text-center text-sm p-3 cursor-not-allowed" />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input required type="text" id="justification" name="justification" placeholder="State clear drivers for creating or modifying this procedure..." value="{{ old('justification') }}" class="w-full rounded-lg border-gray-200 text-sm p-3 placeholder-gray-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all duration-150" />
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 pt-1">
                            <div>
                                ...
                                <textarea id="objective" name="objective" rows="3" placeholder="e.g.,&#10;- To define guidelines for processing records...&#10;- To establish accountability metrics..." class="w-full rounded-lg border-gray-200 text-sm p-3 placeholder-gray-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all duration-150">{{ old('objective') }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="scope" name="scope" rows="3" placeholder="e.g.,&#10;- Covers all internal audit operations...&#10;- Applicable to document controllers across branches..." class="w-full rounded-lg border-gray-200 text-sm p-3 placeholder-gray-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all duration-150">{{ old('scope') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div id="interfaces-outputs-wrapper" class="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                                        @for ($i = 0; $i < 3; $i++)
                                            <div class="flex gap-1.5 items-center w-full">
                                                ...
                                                <div class="relative flex-2">
                                                    <input type="text" class="interface-input-name w-full rounded-md border-gray-200 text-xs p-1.5 placeholder-gray-300 autocomplete-interface" placeholder="Generated Form / Record #{{ $i + 1 }}" autocomplete="off" />
                                                    <div class="autocomplete-suggestions absolute z-50 left-0 mt-1 hidden max-h-48 overflow-y-auto bg-white border border-gray-200 rounded-md shadow-xl"></div>
                                                </div>
                            ...
                            <select id="affected_doc_select" class="w-full rounded-lg border-gray-200 text-sm p-2.5 bg-white shadow-sm focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition">
                                ...
                                @if(isset($all_documents))
                                    @foreach($all_documents as $doc)
                                        <option value="{{ $doc->id }}" data-code="{{ $doc->code }}" data-title="{{ $doc->title }}">{{ $doc->code }} - {{ $doc->title }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // event listeners
            document.getElementById('add-affected-doc-btn').addEventListener('click', addAffectedDocument);
            ...
            // Initialize historical data pools for autocomplete
            const inputsPool = @json($existing_inputs ?? []);
            const outputsPool = @json($existing_outputs ?? []);
            
            const autocompleteConfigs = [
                {
                    wrapper: document.getElementById('interfaces-inputs-wrapper'), pool: inputsPool
                },
                {
                    wrapper: document.getElementById('interfaces-outputs-wrapper'), pool: outputsPool
                }
            ];
            ...
            // Global autocomplete dismissal click-away handler
        ...
        @if (old('definition_of_terms_json'))
            try {
                terms = JSON.parse(@json(old('definition_of_terms_json')));
                renderTermsTable();
                updateDefinitionOfTermsJsonField();
            } catch (e) {
                terms = [];
            }
        @endif

        // Handling definition of terms matrix
        function addTerm() {
        ...
        function updateDefinitionOfTermsJsonField() {
            document.getElementById('definition-of-terms-json').value = JSON.stringify(terms);
        }
        // -------- End of definition of terms script --------

        // Handling affected documents matrix
        function addAffectedDocument() {
            ...
            if (!docId) {
                alert('Please select a target master document from the dropdown list.');
                return;
            }

            if (affectedDocs.some(d => d.id === docId)) {
                alert('This specific document record has already been added to the matrix.');
                return;
            }
            ...
            affectedDocs.push({ id: docId, title, code, revision, details });
            ...
            renderAffectedDocsTable();
        }

        function removeAffectedDocument(index) {
            affectedDocs.splice(index, 1);
            renderAffectedDocsTable();
        }

        function renderAffectedDocsTable() {
            const tbody = document.getElementById('affected-docs-tbody');
            const hiddenContainer = document.getElementById('hidden-affected-inputs-container');
            ...
            affectedDocs.forEach((doc, index) => {
                ...
                tr.innerHTML = `
                    <td class="px-3 py-3.5 text-center text-gray-400 font-bold border-r border-gray-100">${index + 1}</td>
                    <td class="px-4 py-3.5 text-gray-900 font-semibold tracking-tight whitespace-normal break-words">${doc.title}</td>
                    <td class="px-4 py-3.5 text-gray-700 font-mono font-bold">${doc.code}</td>
                    <td class="px-4 py-3.5 text-center text-gray-700 font-medium">${doc.revision || '<span class="text-gray-300 italic font-normal text-xs">None</span>'}</td>
                    <td class="px-4 py-3.5 text-gray-600 whitespace-normal break-words leading-relaxed">${doc.details || '<span class="text-gray-300 italic text-xs">No modification constraints specified</span>'}</td>
                    <td class="px-4 py-3.5 text-center">
                        <button type="button" class="text-gray-400 hover:text-red-500 p-1 rounded-md hover:bg-gray-100 transition text-sm" title="Remove" onclick="removeAffectedDocument(${index})"><i class="fa-solid fa-xmark"></i></button>
                    </td>
                `;
                tbody.appendChild(tr);

                // Map keys  back into backend payload 
                hiddenContainer.insertAdjacentHTML('beforeend', `
                    <input type="hidden" name="affected_documents[${index}][document_id]" value="${doc.id}">
                    <input type="hidden" name="affected_documents[${index}][title]" value="${doc.title}">
                    <input type="hidden" name="affected_documents[${index}][code]" value="${doc.code}">
                    <input type="hidden" name="affected_documents[${index}][revision_number]" value="${doc.revision}">
                    <input type="hidden" name="affected_documents[${index}][details]" value="${doc.details}">
                `);
            });
        }
        // -------- End of affected documents script --------
